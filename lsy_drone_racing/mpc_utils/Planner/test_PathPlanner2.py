import pytest
import numpy as np
from lsy_drone_racing.mpc_utils.Planner.PathPlanner import PathPlanner2
import matplotlib.pyplot as plt
import time


class TestPathPlanner2:
    def test_create_centerline(self):
        ...
        obstacles = []
        drone_position = np.array([-1, -1, 0])

        planner = PathPlanner2(gate_positions, gate_orientations, obstacles, drone_position)
        path, tangent_vectors = planner.generate_centerline(
            gate_positions, gate_orientations, drone_position
        )

        assert len(path) > len(gate_positions)
        assert len(tangent_vectors) > len(gate_orientations)
        assert np.allclose(path[0], drone_position)
        assert np.allclose(path[1], gate_positions[0])
        assert np.allclose(path[-1], gate_positions[-1])
        assert np.allclose(tangent_vectors[1], planner.compute_gate_normals(gate_orientations)[0])
    ...
    def test_path_planning_with_obstacles(self):
        ...
        # Verify the path starts at the drone position and passes through the gates
        assert np.allclose(path[0], drone_position)
        for i, gate in enumerate(gates):
            gate_pos = np.array(gate["pos"])
            assert np.allclose(path[(i + 1) * 101], gate_pos), (
                f"Path does not pass through gate {i + 1}"
            )
        ...
        print("Path starts at the drone position, passes through all gates, and avoids obstacles.")
        # Visualization
        ...
        new_drone_position = np.array([-2, -2, 0])
        ...
        planner.update_drone_position(new_drone_position)
        new_path, new_tangent_vectors = planner.generate_centerline(
            new_gate_positions, new_gate_orientations, new_drone_position
        )
        end_time = time.time()
